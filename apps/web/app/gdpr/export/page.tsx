...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: dict.legal.meta.exportTitle.replace("{clientName}", config.clientName),
      ...
      <div style={{ maxWidth: "620px", margin: "0 auto" }}>
        <div style={{
          ...
          <div style={{
            ...
            <a
              href={`mailto:${email}?subject=${encodeURIComponent(e.mailtoSubject)}`}
          ...
          <Link href="/datenschutz" className="btn btn-ghost btn-sm">{e.backLink}</Link>
        </div>
      </div>
    </div>
  );
